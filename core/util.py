...
def tensor2img(tensor, out_type=np.uint8, min_max=(-1, 1), norm=True):
    ...
    if out_type == np.uint16:
        if norm:
            img_np = (((img_np + 1) / 2) * 65535).round()
        else:
            img_np = (img_np * 65535).round()
        # Important. Unlike matlab, numpy.unit8() WILL NOT round by default.
    return img_np.astype(out_type).squeeze()


def postprocess(images, out_type=np.uint8, min_max=(-1, 1), norm=True):
    return [tensor2img(image, out_type, min_max, norm) for image in images]
