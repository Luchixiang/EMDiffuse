...
class Network(BaseNetwork):
    def __init__(self, unet, beta_schedule, norm=True, module_name='sr3', **kwargs):
        ...
        from .guided_diffusion_modules.unet_3d_aleatoric import UNet
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8, path=None, adjust=False):
        ...
        predict_variance_out = torch.zeros_like(y_t)
        print(predict_variance_out.shape)
            ...
            if i in variance_times:
                ...
                for c in range(1, channel_num):
                    channel_index = torch.cat(
                        [channel_index, torch.full((b,), c, device=y_cond.device, dtype=torch.long)], dim=0)
                noise_level = extract(self.gammas, t, x_shape=(1, 1)).to(y_t.device)
                predict_variance = self.denoise_fn(torch.cat([y_cond, torch.mean(y_cond, dim=1, keepdim=True)], dim=1), noise_level, channel_index, variance=True)
        ...
        predict_variance_outs = predict_variance_out[:b]
        for c in range(1, channel_num):
            predict_variance_outs = torch.cat([predict_variance_outs, predict_variance_out[c * b:(c + 1) * b]], dim=1)
        return  y_t, ret_arr, predict_variance_outs / len(variance_times)
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None,  t=None, times=None, noise_hat=None, sample_gammas=None, variance=False, channel_index=None):
        ...
        if not variance:
            ...
            return noise_hat, noise, sample_gammas, channel_index
            # print(noise.shape, noise_hat.shape)
        else:
            noise_variance = self.denoise_fn(torch.cat([y_cond, torch.mean(y_cond, dim=1, keepdim=True)], dim=1), sample_gammas, channel_index, variance=True)
