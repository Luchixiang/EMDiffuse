...
class EmbedConv(nn.Sequential, EmbedBlock):
...
class ResBlock(EmbedBlock):
    ...
    def __init__(
        ...
        self.in_layers0 = nn.Sequential(
            normalization(channels, group_num),
            SiLU(),
        )
        self.in_layers1 = nn.Sequential(nn.Conv2d(channels, self.out_channel, 3, padding=1), )
        ...
        #     normalization(self.out_channel, group_num),
        #     SiLU(),
        ...
        #     ),
        # )
        self.out_layers0 = nn.Sequential(
            normalization(self.out_channel, group_num),
        )
        self.out_layers1 = nn.Sequential(SiLU(),
                                         nn.Dropout(p=dropout),
                                         zero_module(
                                             nn.Conv2d(self.out_channel, self.out_channel, 3, padding=1)
                                         ))
    ...
    def _forward(self, x, emb):
        ...
        if self.use_scale_shift_norm:
            ...
            # h = out_rest(h)
            scale, shift = torch.chunk(emb_out, 2, dim=1)
            h = self.out_layers0(h) * (1 + scale) + shift
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.use_checkpoint = use_checkpoint
        self.norm = normalization(channels, group_num)
    ...
    def _forward(self, x):
        b, c, img_height, img_width = x.shape
        x = x.reshape(b, c, -1)
        qkv = self.qkv(self.norm(x))
        h = self.attention(qkv)
        h = self.proj_out(h)
        return (x + h).reshape(b, c, img_height, img_width)
...
class UNetJit(nn.Module):
    ...
    def __init__(
        ...
        self.group_num = 16 if inner_channel == 16 else 32
        ...
        self.input_blocks = nn.ModuleList([EmbedSequential(EmbedConv(
            nn.Conv2d(in_channel, ch, 3, padding=1)))]
        )
        ...
        for level, mult in enumerate(channel_mults):
            ...
            if level != len(channel_mults) - 1:
                ...
                self.input_blocks.append(
                   EmbedSequential(EmbedSequential(ResBlock(
                        ch,
                        cond_embed_dim,
                        dropout,
                        out_channel=out_ch,
            ...
            for i in range(res_blocks + 1):
                ...
                if level and i == res_blocks:
                    ...
                    layers.append(
                        ...
                        if resblock_updown
                        else EmbedConv(Upsample(ch, conv_resample, out_channel=out_ch))
                    )
                    ds //= 2
                self.output_blocks.append(EmbedSequential(*layers))
                self._feature_size += ch
        self.out = nn.Sequential(
            normalization(ch, self.group_num),
            SiLU(),
            zero_module(nn.Conv2d(input_ch, out_channel, 3, padding=1)),
        )

    def forward(self, x: torch.Tensor, gammas):
        """
