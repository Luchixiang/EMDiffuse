...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        for level, mult in list(enumerate(channel_mults))[::-1]:
            for i in range(res_blocks + 1):
                ...
                self._feature_size += ch
        self.out_variance = nn.Sequential(
    ...
    def forward(self, x, gammas, channel_index, variance=False):
        """
        ...
        channel_index = channel_index.view(-1, )
        emb = self.cond_embed(gamma_embedding(gammas, self.inner_channel))
        channel_emb = self.channel_embed(gamma_embedding(channel_index, self.inner_channel))
        emb = torch.cat([emb, channel_emb], dim=1)
        # emb = emb + channel_emb
