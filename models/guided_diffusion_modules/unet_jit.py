...
class AttentionBlock(EmbedBlock):
    """
    ...
    def _forward(self, x, emb:Optional[torch.Tensor]=None):
        b, c, *spatial = x.shape
        x = x.reshape(b, c, -1)
...
class UNetJit(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList(
            [EmbedConv(nn.Conv2d(in_channel, ch, 3, padding=1))]
        )
