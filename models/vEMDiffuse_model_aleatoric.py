...
class Palette(BaseModel):
    ...
    def save_current_results(self):
        ...
        for idx in range(self.batch_size):
            for i in range(self.gt_image.shape[1]):
                ...
                ret_path.append('Variance_{}_{}'.format(i, self.path[idx]))
                ret_result.append(torch.sqrt(torch.exp(self.variance[idx, i, :,:].detach().float().cpu())))
    ...
    def train_step(self):
        ...
        for train_data in self.phase_loader:
            ...
            self.optG.zero_grad()
            with torch.no_grad():
                noise_hat, noise, sample_gammas, channel_index = self.netG_copy(self.gt_image, self.cond_image, mask=self.mask)
            loss, _ = self.netG(self.gt_image, y_cond=self.cond_image, mask=self.mask, noise_hat=noise_hat, noise=noise,
                                variance=True, sample_gammas=sample_gammas, channel_index=channel_index)
            loss.backward()
            self.optG.step()
        ...
        with torch.no_grad():
            for phase_data in self.phase_loader:
                ...
                if self.opt['distributed']:
                    ...
                    self.output, self.visuals,  self.variance = self.netG.module.restoration(self.cond_image,
                                                                             y_0=self.gt_image,
                                                                             sample_num=self.sample_num)

                else:
                    self.output, self.visuals, self.variance = self.netG.restoration(self.cond_image,
                                                                      y_0=self.gt_image,
                                                                      sample_num=self.sample_num, path=self.path,
                                                                      adjust=adjust)
                self.iter += self.batch_size
