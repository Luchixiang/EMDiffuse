...
def crop(wf_img, save_wf_path, patch_size=256, stride=224):
...
def test_pre(data_root, task='denoise'):
    ...
    for file in os.listdir(data_root):
        ...
        if task == 'denoise':
            crop(wf_file_img, save_wf_path, patch_size=256, stride=224)
        else:
            crop(wf_file_img, save_wf_path, patch_size=128, stride=112)
