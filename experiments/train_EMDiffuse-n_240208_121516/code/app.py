...
from models.EMDiffuse_network import Network
...
def process(input_image, step, progress=gr.Progress()):
    ...
    with torch.no_grad():
        ...
        for i in progress.tqdm(reversed(range(0, timesteps)), desc='sampling loop time step', total=timesteps):
            ...
            y_t, y_0_hat = model.netG.p_sample(y_t, t, y_cond=img, adjust=False)
...
if __name__ == '__main__':
    ...
    opt['world_size'] = 1
    # opt = json.load('./config/EMDiffuse-n-big.json')
    opt['datasets'][opt['phase']]['dataloader']['args']['batch_size'] = 1
    opt['mean'] = 1
    opt['phase'] = 'test'
    ...
    gr.Interface(
        process,
        inputs=[gr.Image(type="pil", label="Input", image_mode='L'), gr.Slider(100, 1000,label='Step numbers', info="The diffusion model's step count directly impacts image quality: more steps lead to higher quality images.")],
