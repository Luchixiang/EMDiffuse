...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    os.makedirs('bioimage_zoo_model_large', exist_ok=True)
    network.save("bioimage_zoo_model_large/bioimagezoo_weights_emdiffuse-n.pt")
    # input_ = np.random.rand(1, 1, 768, 768).astype("float32")  # an example input
    input_ = Image.open('./demo/22.png').convert('L')

    input_ = np.expand_dims(input_, axis=0)
    input_ = np.expand_dims(input_, axis=0).astype(np.float32)
    np.save("bioimage_zoo_model_large/test-input.npy", input_)
    ...
    np.save("bioimage_zoo_model_large/test-output.npy", output)
    with open("bioimage_zoo_model_large/doc.md", "w") as f:
    ...
    build_model(
        ...
        weight_uri="bioimage_zoo_model_large/bioimagezoo_weights_emdiffuse-n.pt",
        weight_type="torchscript",
        # the test input and output data as well as the description of the tensors
        # these are passed as list because we support multiple inputs / outputs per model
        test_inputs=["bioimage_zoo_model_large/test-input.npy"],
        test_outputs=["bioimage_zoo_model_large/test-output.npy"],
        input_axes=["bcyx"],
        output_axes=["bcyx"],
        # where to save the model zip, how to call the model and a short description of it
        output_path="bioimage_zoo_model_large/emdiffuse-n.zip",
        name="EMDiffuse-n",
        ...
        documentation="bioimage_zoo_model_large/doc.md",
        tags=["EMDiffuse", "Electron microscope", "Denoising", "Diffusion Model", "Image restoration", "bioimage.io"],  # the tags are used to make models more findable on the website
        cite=[{"text": "Lu et al.", "doi": "https://doi.org/10.1101/2023.07.12.548636"}],
    )
    from bioimageio.core.resource_tests import test_model
    import bioimageio.core
    my_model = bioimageio.core.load_resource_description("bioimage_zoo_model_large/emdiffuse-n.zip")
    test_model(my_model)
    phase_writer.close()
